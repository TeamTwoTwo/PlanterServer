...
class AuthController(private val authService: AuthService, private val passwordEncoder: PasswordEncoder) {
    ...
    fun login(@Valid @RequestBody userLoginReq: UserLoginReq): BaseResponse<UserLoginRes> {
        ...
        val userLoginRes = UserLoginRes(authService.login(user.id!!), user.id!!, user.email, user.name, user.birth, user.phone,
            user.address, user.detailAddress, user.latitude, user.longitude, user.simpleAddress, user.nickname)
    ...
    fun checkDuplicate(checkDuplicateReq: CheckDuplicateReq): BaseResponse<Any> {
        ...
        if(checkDuplicateReq.phone != null && authService.existsPhone(checkDuplicateReq.phone)) {
            throw BaseException(DUPLICATE_PHONE)
        }
